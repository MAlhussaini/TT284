...
if (!empty($first_name) || !empty($last_name) || !empty($password) || !empty($phone_number) || !empty($email) || !empty($city) || !empty($address)) {
    ...
    if ($conn->connect_error) {
        ...
        $Insert = "INSERT INTO registration (first_name, last_name, password,email, phone, city, address) values(?, ?, ?, ?, ?, ?, ?)";
        ...
        if ($rnum == 0) {
            ...
            if ($stmt->execute()) {
                echo "<script type='text/javascript'>alert('User has been successfully registered'); location.href = 'Form.html';</script>";
            } else {
                echo $stmt->error;
            }
        } else {
